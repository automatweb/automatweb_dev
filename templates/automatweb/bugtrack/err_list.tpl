{VAR:header}

<!-- SUB: fgroup -->
<select name="fgroup{VAR:gid}" OnChange="DoFilter({VAR:gid})" class="small_button">
<option value='0' {VAR:sel}>{VAR:gname}</option>
<!-- SUB: filter -->
<option value='{VAR:fid}' {VAR:sel}>{VAR:fname}</option>
<!-- END SUB: filter -->
</select>
<!-- END SUB: fgroup -->

<form action='orb.{VAR:ext}' method='GET' name='foo'>
<div align="left" class="fgtext">
<!-- SUB: PAGE -->
<a href='{VAR:link}'>{VAR:from} - {VAR:to}</a> | 
<!-- END SUB: PAGE -->

<!-- SUB: SEL_PAGE -->
{VAR:from} - {VAR:to} | 
<!-- END SUB: SEL_PAGE -->

Grupeeri: <input type='radio' name='groupby' value='id' {VAR:grpby_id}> ID <input type='radio' name='groupby' value='type_id' {VAR:grpby_type_id}> T&uuml;&uuml;bi <input type='radio' name='groupby' value='site' {VAR:grpby_site}> Saidi j&auml;rgi. <a href='javascript:foo.submit()'>Uuenda</a>
</div>
<script language="javascript">
sel1_="";
function Do(what)
{
	document.foo.action.value=what;
	document.foo.submit();
};
function DoDelegate()
{
	if (document.foo.sel1 && sel1_)
		{remote(0,300,200,'{VAR:l_delegate}&id='+sel1_);}
		else alert('Midagi pole valitud!');
};
function DoFilter(what)
{
	eval("what=document.foo.fgroup"+what+".value");
	window.location="{VAR:l_setfilter}&_setfilter=1&setfilter="+what;
};
</script>
{VAR:table}
{VAR:reforb}
</form>