...
{
	var params = {};
	$("#props_form").find("input[@checked], input[@type='text'], input[@type='hidden'], input[@type='password'], input[@type='submit'], option[@selected], textarea").filter(":enabled").each(function()
	{
		params[ this.name || this.id || this.parentNode.name || this.parentNode.id ] = this.value;
	})
	$.post('{VAR:form_action}', params, function(xml)
	{
		$('#prop_div').fadeOut(500)
	});
}
</script>
<div style="float: right; position: relative; top: -10px; left: 10px; font-family: arial; font-weight: bold;">
<a href="#" style="color: #000;" id="msg_popup_close">X</a>
</div>
<form id="props_form">
...
<br />
<input type="button" onclick="ajaxSubmit()" value="{VAR:submit_caption}" id="button" />
</form>