<table border=0>
	<tr>
		<td class='awmenuedittabletext'>Kliendi üldandmed</td>
		<td class='awmenuedittabletext'>&nbsp;</td>
		<td class='awmenuedittabletext'>Kliendisuhe</td>
		<td class='awmenuedittabletext'>&nbsp;</td>
	</tr>
	<tr>
		<td class='awmenuedittabletext'>Nimetus ja õiguslik vorm</td>
		<td class='awmenuedittabletext'>{VAR:name}</td>
		<td class='awmenuedittabletext'>Alguskuup&auml;ev;</td>
		<td class='awmenuedittabletext'>{VAR:cust_contract_date}</td>
	</tr>
	<tr>
		<td class='awmenuedittabletext'>Kood</td>
		<td class='awmenuedittabletext'>{VAR:code}</td>
		<td class='awmenuedittabletext'>Suhte looja</td>
		<td class='awmenuedittabletext'>{VAR:cust_contract_creator}</td>
	</tr>
	<tr>
		<td class='awmenuedittabletext'>Registrikood</td>
		<td class='awmenuedittabletext'>{VAR:reg_code}</td>
		<td class='awmenuedittabletext'>Sissetuleku meetod</td>
		<td class='awmenuedittabletext'>{VAR:referal_type}</td>
	</tr>
	<tr>
		<td class='awmenuedittabletext'>KMK nr</td>
		<td class='awmenuedittabletext'>{VAR:kmk_nr}</td>
		<td class='awmenuedittabletext'>Kliendihaldur</td>
		<td class='awmenuedittabletext'>{VAR:client_manager}</td>
	</tr>
	<tr>
		<td class='awmenuedittabletext'>Tegevuse kirjeldus</td>
		<td colspan="3" class='awmenuedittabletext'>{VAR:desc}</td>
	</tr>
	<tr>
		<td class='awmenuedittabletext'>Kaubam&auml;rgid</td>
		<td colspan="3" class='awmenuedittabletext'>{VAR:trademarks}</td>
	</tr>

	<tr>
		<td colspan="4">&nbsp;</td>
	</tr>
	<tr>
		<td class='awmenuedittabletext'>Kontaktandmed</td>
		<td class='awmenuedittabletext'>&nbsp;</td>
		<td class='awmenuedittabletext'>Kontaktisikud</td>
		<td class='awmenuedittabletext'>&nbsp;</td>
	</tr>
	<tr>
		<td class='awmenuedittabletext'>Aadress</td>
		<td class='awmenuedittabletext'>{VAR:address}</td>
		<td valign="top" rowspan="5" colspan="2" class='awmenuedittabletext'>
			{VAR:contact_p}
		</td>
	</tr>
	<tr>
		<td class='awmenuedittabletext'>Telefon</td>
		<td class='awmenuedittabletext'>{VAR:phone}</td>
	</tr>
	<tr>
		<td class='awmenuedittabletext'>Faks</td>
		<td class='awmenuedittabletext'>{VAR:fax}</td>
	</tr>
	<tr>
		<td class='awmenuedittabletext'>E-post</td>
		<td class='awmenuedittabletext'>{VAR:email}</td>
	</tr>
	<tr>
		<td class='awmenuedittabletext'>WWW</td>
		<td class='awmenuedittabletext'>{VAR:web}</td>
	</tr>
</table>