<?php
$GLOBALS["TRANS"]["Omaduse Piltide kataloog (folder) caption"] = "Image folder";
$GLOBALS["TRANS"]["Omaduse Tulpi (cols) caption"] = "Columns";
$GLOBALS["TRANS"]["Omaduse Ridu (rows) caption"] = "Rows";
$GLOBALS["TRANS"]["Omaduse Pildid kommenteeritavad (comments) caption"] = "Enable to comment the images";
$GLOBALS["TRANS"]["Omaduse Piltide stiil (style) caption"] = "Image style";
$GLOBALS["TRANS"]["Omaduse Uploadi ZIP fail (zip_file) caption"] = "Upload ZIP file";
$GLOBALS["TRANS"]["Grupi Import (import) pealkiri"] = "Import";
$GLOBALS["TRANS"]["Seose Piltide kataloog (RELTYPE_IMG_FOLDER) tekst"] = "image folder";
$GLOBALS["TRANS"]["Seose Stiil (RELTYPE_STYLE) tekst"] = "style";
$GLOBALS["TRANS"]["Nimi"] = "Name";
$GLOBALS["TRANS"]["Valitud piltide kataloogi ei ole &otilde;igusi objekte lisada!"] = "No add access to the selected image folder";
$GLOBALS["TRANS"]["Galerii "] = "Gallery";
$GLOBALS["TRANS"][" pildid"] = " images";
?>