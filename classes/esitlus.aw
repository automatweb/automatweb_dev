...
global $orb_defs;
$orb_defs["esitlus"] = "xml";
//define("SHOW_TPL_DIR",aw_ini_get("tpldir")."/esitlus/show");

class esitlus extends aw_template
{
	////////////////////////////////////
	...
	function add($arr)
	{
		...
		if ($return_url != "")
		{
			$this->mk_path(0,"<a href='$return_url'>Tagasi</a> / Leia tooteid");
		}
		else
		{
			$this->mk_path($parent,"Lisa esitlus");
		}
		...
		$this->vars(array(
			"name" => "esitlus",
			"joon" => "<hr>",
			"pealkiri" => "AutomatWebi tooted",
        	"rootitems" => $this->picker(0, $this->get_menu_list()),
		...
		$ret = $this->list_objects(array("parent" => $parent));
	...
	function change($arr)
	{
		...
		if ($return_url != "")
		{
			$this->mk_path(0,"<a href='$return_url'>Tagasi</a> / Muuda esitlus");
		}
		else
		{
			$this->mk_path($ob["parent"], "Muuda esitlus");
		}
		$this->read_template("change.tpl");
	
		$this->vars(array(
			"name" => $ob["name"],
			"newwindow" => checked($products["newwindow"]),
...
/*	////
	// to open the products documents in a new window
