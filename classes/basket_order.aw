...
class basket_order extends basket
{
	...
	**/
	function change($arr)
	{
		...
		{
			$tmp = $order->meta("form_ids");
			$prc = $order->meta("prices");
			$fid = $tmp[$iid];
			...
			$this->vars(array(
				...
				"count" => $icnt,
				"it_price" => $prc[$iid] * $icnt,
				"price" => $prc[$iid]
			));
			$its.=$this->parse("ITEM");
			$t_count += $icnt;
			$t_price += $prc[$iid] * $icnt;
		}
		...
		else
		{
			...
			$htmlmail = $finst->show(array(
				...
				"op_id" => $order->meta("order_form_op")
			));
	...
	**/
	function another($arr)
	{
		...
		foreach($order->meta("items") as $iid => $icnt)
		{
			...
			$this->set_item_count(array("item_id" => $iid, "form_id" => $tmp[$iid], "count" => $icnt));
			$this->set_item_price(array("item_id" => $iid, "price" => $prc[$iid]));
		}
