...
class forell extends aw_template
{
	////////////////////////////////////
	...
	function add($arr)
	{
		...
		if ($return_url != "")
		{
			$this->mk_path(0,"<a href='$return_url'>Tagasi</a> / Lisa forell");
		}
		else
		{
			$this->mk_path($parent,"Lisa forell");
		}
	...
	function change($arr)
	{
		...
		if ($return_url != "")
		{
			$this->mk_path(0,"<a href='$return_url'>Tagasi</a> / Muuda forell");
		}
		else
		{
			$this->mk_path($ob["parent"], "Muuda forell");
		}
		$this->read_template("change.tpl");
	
		$this->vars(array(
			"name" => $ob["name"],
			"shout_it" => $this->mk_my_orb("shout-action", array("arg" => "See karjutav text!!")),
	...
	{
		extract($sentence); // teeb kohaliku muutja mille nimi on arg (orbi defist)
		$this->read_template("shout.tpl");
		$this->vars(array(
			"shout_text" => $arg
		));
		return $this->parse();
