...
// siit algab messengeri pıhiklass
class messenger extends menuedit_light 
{
	...
	function messenger($args = array())
	{
		...
		// $this->driveri kaudu pˆˆrdutakse andmebaasidraiveri poole
		$this->driver = new $driverclass;
		
		// juhuks, kui kusagil on vaja kasutada messengeri alamhulka, siis konstruktorile
		// fast argumendi etteandmisega saab v‰ltida rohkem aega nıudvaid operatsioone
		// kuigi tegelikult peaks selleks messengeri klassi hoopis kaheks lˆˆma
		// messenger_user ja messenger. Hiljem jıuab. Ehk.
		if (!$args["fast"])
		{
	...
	function pick_folder($args = array())
	{
		...
		{
			$this->vars(array(
					"folder" => "<a href='$baseurl/?class=messenger&action=pick_folder&id=$row[oid]&type=popup&attach=$attach&msg_id=$msg_id'>$row[name]</a>",
			));
			$c .= $this->parse("line");
		};
		$u = "";
	...
	////
	// !Pritsib attachi kasutajale v‰lja
	// argumendid:
	// msgid(int) - kirja id
	...
	////
	// !Attachib objekti mingi kirja k¸lge
	// msg_id - teade mille k¸lge attachide
	...
	// argumendid
	// check(array) - m‰rgistatud teadete ID-d
	...
	function mailbox_op($args = array())
	{
		...
		// Ja avame j‰lle selle folderi, mida me enne vaatasime
		return $this->mk_site_orb(array(
			"action" => "folder",
			"id" => $active_folder,
		));
	...
	function gen_edit_form($args = array())
	{
		...
		// picker funktsioonidel on 0 v‰‰rtusega elementidega kalad
	...
	function post_message($args = array())
	{
		...
		// ja lopuks liigutame ta draftist ‰ra outboxi
	...
	function do_search($args = array())
	{
		...
		for ($i = 1; $i < sizeof($search); $i++)
		{
			...
			if ($search[$i - 1])
			{
				...
				$quser .= sprintf(" ..v‰ljal <b>%s</b> sisaldub string '%s'",$this->fields[$field[$i - 1]],stripslashes(htmlspecialchars($search[$i - 1])));
			};
	...
	function show_message($args = array())
	{
		...
		// id-d kasutame sellep‰rast, et integeri jargi otsimine on kiirem, kui 
		// stringi (ntx UIDL)
		...
		// Sıltuvalt message t¸¸bist on vaja erinevad template v‰ljad t‰ita erinevate v‰‰rtustega
		switch($msg["type"])
		{
			case MSG_EXTERNAL:
		...
		// kui kasutaja soovib teateid liigutada, siis teeme seda n¸¸d
		$moveto = ($this->msgconf["msg_move_read"]) ? $this->msgconf["msg_move_read_folder"] : "";
		$this->driver->msg_mark(array(
						"id" => $id,
						"parent" => $moveto,
	...
	////
	// !Moodustab kasutaja messengeri konfiguratsiooni, asendades puuduvad v‰‰rtused vajadusel defaultidega
	// conf - users tabelist loetud "messenger" v‰lja sisu
	function _get_msg_conf($args = array())
	{
		...
		// k¸sida kirja saatmisel kinnitust
		...
		// millist m‰rki vaikimisi kvootimiseks kasutatakse
	...
	function configure($args = array())
	{
		...
		// messengeri nimelises v‰ljas kasutaja tabelis hoiakse kasutaja mailboxi konfiguratsiooni
		// xml-is, if you want to know.

		// kui kasutajal mailboxi konff puudub, siis xml_unserialize tagastab t¸hja array
		...
		switch($page)
		{
			...
			case "signature":
				// Kıigepealt koostame olemasolevate signatuuride nimekirja
				$siglist = "";
				$cnt = 0;
				if (is_array($this->msgconf["msg_signatures"]))
				{
		...
		// page abil suuname kasutaja p‰rast tagasi ıigele lehele
		$this->vars(array(
				"content" => $content,
				"menu" => $menu,
	...
	function submit_configure($args = array())
	{
		...
		// n¸¸d ts¸kkel ¸le kıigi $args-i elementide, mille nimi algab msg_-ga
	...
	function _submit_configure($args = array())
	{
		...
		else
		{
			...
			// Salvestamisel k¸sime kıigepealt messengeri esialgse konfiguratsiooni	
			$conf = $this->_get_msg_conf(array(
								"conf" => $this->user["messenger"],
							));
			$_aq = "";
			...
			// overraidime vanas konfis olnud v‰‰rtused vormist tulnutega
			...
			// kustutame m‰rgitud signatuurid
			if (is_array($delsig))
			{
			...
			// lisame ainult siis, kui nii nimi, kui ka sisu on m‰‰ratud
	...
	////
	// !Kuvab uue accoundi t¸¸bi valimise vormi
			...
			foreach($msgs as $data)
			{
				...
				if ($res > 0)
				{
					for ($i = 1; $i <= $res; $i++)
					{
						...
						{
							// kui me attachi ei salvestanud, siis votame p‰rast $res-i vaiksemaks,
				...
				// tyypi 2 on v‰lised kirjad. as simpel as that.
	...
	function editrule($args = array())
	{
		...
		if (isset($id))
		{
			...
			// juhuks, kui tabelis see v‰li t¸hjaks on j‰‰nud. Mida muidugi ei tohiks juhtuda
