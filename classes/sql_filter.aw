...
class sql_filter extends aw_template 
{
	...
	function sql_filter()
	{
		...
		array("BugTrack"=>array(
			...
			"fields"=>array(
				...
				"Filtri t¸¸p"=>array("real"=>"ftype","type"=>1,"select"=>array(0=>"null",1=>"yks",2=>"kax")))
			))
		*/
	...
	function filter_to_sql($arr=array())//noeval kas evalida @uid ja @t... ,fake kas kasutada fake v‰lju
	{
		extract($arr);
		...
		for ($i=0; $i<(int)$fi["nump"]; $i++)
		{
			...
			//done
//mysql>vali parool seest maiesskuell.user kus noo ma tahaks nagu neid kus nimi oleks ikka veits pikem kui 5 m‰rki vıi nii, noh!
