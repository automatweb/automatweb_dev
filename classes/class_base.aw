...
class class_base extends aw_template
{
	...
	**/
	...
	function get_cfgform_for_object($args = array())
	{
		...
		$interfaces = class_implements($application, true);
		if (in_array("application_interface", $interfaces))
		{
	...
	function callback_generate_scripts_from_class_base($arr)
	{
		...
		if(count($draftable_props) > 0)
		{
			...
			$odl = new object_data_list(
				$params,
				array(
					CL_DRAFT => array("draft_property")
				)
			);
			$drafted_props = safe_array($odl->get_element_from_all("draft_property"));
			$function_draft = "
			...
			{
				$function_draft .= "
				var drafted_props = new Array('".implode("','", $drafted_props)."');
				if(confirm('".t("Teil on selle objekti omaduste jaoks mustandid. Kas soovite need laadida?")."'))
				{
