...
@default group=web_settings
	...
	@caption Dokumendi ID, milles asub kalendri vaade, milles s¸ndmusi kuvatakse

	@property skip_subproject_events type=checkbox ch_value=1 table=aw_projects field=aw_skip_subproject_events
	@caption ƒra n‰ita alamprojektide s¸ndmusi
...
@groupinfo trans caption="Tılkimine"
...
*/			case "transl":
	...
	**/
	...
	function get_events($arr)
	{
		...
		if (is_array($this->prj_map))
		{
			...
			// aga valik "n‰ita alamprojektide s¸ndmusi" ei oma ju ¸le¸ldse mitte mingit mıtet
			// kui mul on vennad kıigis ¸lemprojektides ka
			foreach($this->prj_map as $key => $val)
			{
				// nii . aga n¸¸d ta n‰itab mulle ju ka master projektide s¸ndmusi .. which is NOT what I want

				// teisisınu - mul ei ole s¸ndmuste lugemisel vaja kıiki peaprojekte
		...
		*/
		...
		while($row = $this->db_next())
		{
			...
			// ‰kki ma saan siis siin ka kasutada seda ts¸klite ¸hendamist?
		...
		// nii .. ¸hesınaga me diilime kogu aeg originaalprojektidega siin. eks?
		$conns = $c->find(array(
			"to" => $projects,
		...
		*/
		...
		foreach($conns as $conn)
		{
			...
			if (aw_global_get("uid") == "meff")
			{
				...
				// ja mida ma teen, kui ei ole? Kuidas ma saan selle ıige asja leida?
		...
		if (1 == $arr["project_media"])
		{
			...
			foreach($conns as $conn)
			{
				...
				//$v_o = $conn->to();
				// aga miks siis see asi ei anna mulle tılget ıiges keeles?
				$tmp = $v_o->properties();
		...
		*/
		...
		if (sizeof($events) > 0)
		{
			...
			// iga eventi kohta on vaja teada kıiki vendi
			$ol = new object_list(array(
				"brother_of" => $ids,
				"lang_id" => array(),
			));
			...
			foreach($ox as $brot)
			{
				...
				// et siis teeme uue nimekirja kıigist objektidest, jees?
	...
	**/
	...
	// seega .. alustades ¸hest projektist leiame kıik selle projekti alamprojektid
	...
	function callback_get_add_event($args = array())
	{
		...
		if (isset($clid))
		{
			if (!in_array($clid,$this->event_entry_classes))
			{
				return array(array(
					"type" => "text",
					"value" => t("Seda klassi ei saa kasutada s¸ndmuste sisestamiseks"),
				));
			}
			else
			{
	...
	function request_execute($o)
	{
		...
		// argh .. projekti otse vaatamin on ikka paras sitt k¸ll
		...
		*/
		...
		/*
			...
			[15:18] <duke> oki, loen siis kıik seosed ja vıtan ise need mis mul vaja on
		*/
	...
	**/
	...
	{
		// saan ette project id, alguse ja lıpu
		$rv = array();
		$ol = new object_list(array(
			"parent" => $arr["id"],
			"sort_by" => "planner.start",
