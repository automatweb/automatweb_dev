...
class shop_rent_configuration_obj extends shop_matrix_obj
{
	...
	**/
	public function valid_conditions($arr)
	{
		...
		enter_function("shop_rent_configuration_obj::valid_conditions");

		// Prepare the arguments for price evaluation code
		$args = self::handle_arguments($arr);
		$retval = $this->run_rent_conditions_evaluation_code($args);
		exit_function("shop_rent_configuration_obj::valid_conditions");
		return $retval;
	}
	...
	public function update_code()
	{
		...
		$odl = new object_data_list(
			...
			array(
				CL_SHOP_RENT_CONDITIONS => array("row", "col", "currency", "min_amt", "max_amt"),
			)
		);
	...
	protected function run_rent_conditions_evaluation_code($args)
	{
