...
*/
	...
	**/
		...
		foreach ($sites as $site => $order)
		{
			...
			switch ($site)
			{
				...
				case "baur":
					...
					break;
				case "heine_2":
					$picture_found = $this->heine_ftp_picture_import($arr);
					break;
				case "venca":
					$picture_found = $this->venca_picture_import($arr);
					break;
				case "mecro":
					$picture_found = $this->mecro_ftp_picture_import($arr);
					break;
				case "sandman":
					$picture_found = $this->sandman_ftp_picture_import($arr);
					break;
			}
	...
	function venca_picture_import($arr)
	{
		$pcode = str_replace(' ', '', $arr['pcode']);
		$pcode = str_replace('.', '', $arr['pcode']);
		$pcode = trim($pcode);

		$file_url='/www/www.bonprix.ee/public/venca/linkage.txt';
		
		echo "-- Searching pictures from Venca file ".$file_url." for - ".$pcode." :<br />\n";
		...
		foreach ($f as $line)
		{
			...
			// - the line $l has to be ltrim()-d before passed to mb_convert_encoding() fn. I think it removes the null byte in front of the line.
			// - the from encoding has to be precisely UTF-16LE 
			//$l = mb_convert_encoding(ltrim($line), "UTF-8", "UTF-16LE");
			if (!empty($line))
			{
				$result = explode("\t", $line);
			}
			$subcode=$result[0];
			$subcode=substr($subcode,1);
			$code=$result[1].$subcode;
			if($code==$pcode)
			{
				$url="http://www.venca.es/img/imgprodAn/".$result[20].$result[21]."An".$subcode.".jpg";
				$url2="http://www.venca.es/img/imgprodAr/".$result[20].$result[21]."Ar".$subcode.".jpg";
				$url3="http://www.venca.es/img/imgprodAz/".$result[20].$result[21]."Az".$subcode.".jpg";
				if(!in_array($url, $result_images)&&file_get_contents($url))
				{
					$result_images[]=$url;
				}
				if(!in_array($url2, $result_images)&&file_get_contents($url2))
				{
					$result_images[]=$url2;
				}
				if(!in_array($url3, $result_images)&&file_get_contents($url2))
				{
					$result_images[]=$url3;
				}
				if(empty($result_images))
				{
					$url="http://www.venca.es/img/imgprodAn/".$result[20].$result[21]."An.jpg";
					$url2="http://www.venca.es/img/imgprodAr/".$result[20].$result[21]."Ar.jpg";
					$url3="http://www.venca.es/img/imgprodAz/".$result[20].$result[21]."Az.jpg";
					if(!in_array($url, $result_images)&&file_get_contents($url))
					{
						$result_images[]=$url;
					}
	...
	function sandman_ftp_picture_import($arr)
	{
		$pcode = str_replace(' ', '', $arr['pcode']);
		$pcode = trim($pcode);
		echo "-- Searching pictures from Sandman FTP for product - ".$pcode." :<br />\n";
		$start_time = $arr['start_time'];
		$import_obj = $arr['import_obj'];
		
		$site='www.bonprix.ee/public/vv_sandman';
		$directory='/www/'.$site;
		$site_url=str_replace('public/', '', $site);
		$url='http://'.$site_url."/";
		
		$result_images = array();
		
		$files=scandir($directory);
		$temp=array_shift($files);
		$temp=array_shift($files);

		$result_images = $this->scan_filearray($pcode, $files, $url);
	...
	function mecro_ftp_picture_import($arr)
	{
		$pcode = str_replace(' ', '', $arr['pcode']);
		$pcode = trim($pcode);
		echo "-- Searching pictures from Mecro FTP for product - ".$pcode." :<br />\n";
		$start_time = $arr['start_time'];
		$import_obj = $arr['import_obj'];
		
		$site='www.bonprix.ee/public/vv_mecro';
		$directory='/www/'.$site;
		$site_url=str_replace('public/', '', $site);
		$url='http://'.$site_url."/";
	...
	function heine_ftp_picture_import($arr)
	{
		$pcode = str_replace(' ', '', $arr['pcode']);
		$pcode = trim($pcode);
		echo "-- Searching pictures from Heine FTP for product - ".$pcode." :<br />\n";
		$pcode_firstnumber=substr($pcode,0,1);
		$start_time = $arr['start_time'];
		$import_obj = $arr['import_obj'];
		
		$site='www.bonprix.ee/public/vv_heine';
		$directory='/www/'.$site.'/'.$pcode_firstnumber;
		$site_url=str_replace('public/', '', $site);
		$url='http://'.$site_url.'/'.$pcode_firstnumber."/";
	...
	function scan_filearray($pcode, $files, $url)
	{
		...
		{
			$length_without_extension=strlen($file)-strlen(strrchr($file, "."));
			$temp_file=substr($file, 0, $length_without_extension);
			$temp_arr = explode("_", $temp_file);
			if($temp_arr[0]==$pcode)
			{
				$image_url = $url.$file;
				if(file_get_contents($image_url)!==false)
				{
					echo "Image found for product ".$pcode." <br>\n";
					echo $image_url."<br>\n";
