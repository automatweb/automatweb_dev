<?php
// $Header: /home/cvs/automatweb_dev/classes/applications/shop/otto/otto_import.aw,v 1.47 2007/06/25 13:17:46 dragut Exp $
...
/*
	//	$url = "http://www.bonprix.pl/katalog.php?ss=".$pcode;
		$url = "http://www.bonprix.pl/bp/offer.htm?s=".$pcode;
		$html = $this->file_get_contents($url);
		...
		if (strpos($html, "Niestety, ale nie ma") === false)
		{
			...
			preg_match_all("/http:\/\/img.bonprix.pl\/(\d+)\/(.*).jpg/", $html, $mt, PREG_PATTERN_ORDER);
...
//		if ($o->prop("restart_pict_i"))
		...
		{
			echo "Restarting pictures import (by deleting /files/status.txt file)";
...
//echo dbg::dump($row);
		...
		foreach(explode("\n", $o->prop("fnames")) as $fname)
		{
			...
			while ($row = fgetcsv($fp,1000,"\t"))
			{
				...
				{
					$not_found_products_by_page[$cur_pg][$row[4]] = $row;
					$prod_title = $this->db_fetch_field("select title from otto_imp_t_prod where pg='".$cur_pg."' and nr='".$row[1]."'", "title");
					$not_found_products_by_page[$cur_pg][$row[4]][] = $prod_title;
					echo "X";
