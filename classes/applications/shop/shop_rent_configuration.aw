<?php
/*
@classinfo syslog_type=ST_SHOP_RENT_CONFIGURATION relationmgr=yes no_comment=1 no_status=1 prop_cb=1 maintainer=instrumental
@tableinfo aw_shop_rent_configuration master_index=brother_of master_table=objects index=aw_oid
...
class shop_rent_configuration extends shop_matrix
{
	...
	{
		$this->init(array(
			"tpldir" => "applications/shop/shop_rent_configuration",
			"clid" => CL_SHOP_RENT_CONFIGURATION
		));
	}
...
//				"props" => array(),
	...
	function do_db_upgrade($t, $f)
	{
		if ($f == "")
		{
			$this->db_query("CREATE TABLE aw_shop_rent_configuration(aw_oid int primary key)");
			return true;
		}
