...
class crm_task_list extends object_list
{
	...
	protected $task_classes = array(//!!! teha get_class_label() v6i uurida kas on olemas juba
		CL_TASK => "CL_TASK",
...
**/
	public function filter($param, $task_list_param = array())
	{
		...
		{ // special properties only if in sales application
			...
			if (!empty($task_list_param["own_tasks_only"]) or crm_sales_obj::ROLE_SALESMAN == $role)
			{
				...
				{
					$clid_label = $this->task_classes[$clid];
					$participant_param1["{$clid_label}.RELTYPE_ROW.RELTYPE_IMPL"] = $current_person;
