...
//			"date" => new obj_predicate_compare(OBJ_COMP_BETWEEN, $time_stats["from"]-1, ($time_stats["to"] + 86399)),
		...
		{
			$filter["CL_TASK_ROW.RELTYPE_IMPL.name"] = "%".$req["stats_s_worker"]."%";
		}
...
//			$ol->sort_by(array("prop" => "date","order" => "asc"));
		...
		foreach($ol->arr() as $o)
		{
			...
			foreach($o->connections_from(array("type" => "RELTYPE_IMPL")) as $conn)
			{
				$impl.= html::href(array(
					"url" => "javascript:gt_change(".$conn->prop("to").");" ,
...
*/
	...
	private function get_tasks_filter($arr)
	{
		...
		if($arr["person"])
		{
			$filter["CL_CRM_CALL.RELTYPE_ROW.RELTYPE_IMPL"] = $arr["person"];
		}
		return $filter;
	}
	...
	private function get_meetings_filter($arr)
	{
		...
		if($arr["person"])
		{
			$filter["CL_CRM_MEETING.RELTYPE_ROW.RELTYPE_IMPL"] = $arr["person"];
		}
		if($arr["done"] == 1)
		{
			$filter["is_done"] = $arr["done"];
