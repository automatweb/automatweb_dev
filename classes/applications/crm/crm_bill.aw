...
//				break;
			...
			case "sum":
				...
				if(isset($agreement_prices[0]) && $agreement_prices[0]["price"] && strlen($agreement_prices[0]["name"]) > 0)
				{
					$sum = 0;
...
/*		$t->define_chooser(array(
	...
	**/
	...
	//templeidi lugemine
		...
		{
			$lc = $b->prop("language.lang_acceptlang");
		}
		...
		if ($this->can("view", $b->prop("customer")))
		{
			...
			if ($this->can("view", $ord->prop($prop)))
			{
				...
				if($b->prop("language"))
				{
					...
					if($lc == "en")
					{
						$ord_country = $b->get_customer_address("country_en");
					}
				}
				else
				{
					$ord_country = $b->get_customer_address("country");
				}
			}
		}
	//tellija l6pp
	...
	//teostaja l6pp
		...
		$ord_county = $b->get_customer_address("county");

		$this->vars(array(
			"orderer_name" => $b->get_customer_name(),
			"orderer_code" => $b->get_customer_code(),
			...
			"orderer_addr" => $b->get_customer_address()." ".$ord_index,
			"orderer_city" => $b->get_customer_address("city"),
			"orderer_county" => $ord_county,
			"orderer_index" => $b->get_customer_address("index"),//$ord_index,
			"orderer_country" => $ord_country,
			"orderer_street" => $b->get_customer_address("street"),//$ord_street,
			...
			"overdue" => $b->get_overdue_charge(),
			"bill_text" => $b->get_bill_text(),
		));
		if($ord_country)
		{
			$this->vars(array("HAS_COUNTRY" => $this->parse("HAS_COUNTRY")));
		...
		foreach($brows as $row)
		{
			...
			$grp_rows[$row["prod"]][$unp]["tot_cur_sum"] +=  empty($cur_sum) ? 0 : $cur_sum;
...
//				}
				$this->vars(array(
					"unit" => $b->get_unit_name($grp_row["unit"]),
...
//			$tax_rate = $there_is_tax_rate;
	...
	//templeidi valik l6pp
		...
		$this->vars($this->bill_vars($this->bill));
		...
		{
			$this->vars(array("tot_amt" => $this->stats->hours_format($tot_amt)));
