	...
	function _get_control($arr)
	{
		...
		if(strlen($o->name())>0)
		{
			$s_link = html::href(array(
				...
				"caption" => "mängi",
				"onclick" => 'myRef = window.open("'.$this->get_play_url($o->id(), $s_filename).'","AW MP3 Mängija","left="+((screen.width/2)-(250/2))+",top="+screen.height/5+",width=250,height=350,toolbar=0,resizable=0,location=0,directories=0,status=0,menubar=0,scrollbars=0")',
			));
	...
	**/
	function playlist($arr)
	{
		...
		for ($o = $ol->begin(); !$ol->end(); $o =& $ol->next())
        {
			...
			$s_out .= "			<identifier>".$o->prop("id")."</identifier>";
			$s_out .= "		</track>\n";
		}
