...
//			if (($j%2)&&$j!=0) $values[$j]="";
	...
	function parseData($xvalues,$yvalues) 
	{
		...
		$min=999999999999999999999999;
			//Siin siis v6etakse y v‰‰rtused ette.
				while(list(,$v) = each($yvalues)) 
				{
					//Kui on array kurrentne y v‰‰rtus
					if (is_array($v))
					{
						//siis vaatame mis v2rk on siis selle v‰‰rtusega mis array oli
...
//							echo "$ke = $val<br>";
							//kui see ka veel array on siis on ikka p‰ris perse lahti. lasta siis olla. miski debug v2rk vist :)
							//see on tegelikult p‰ris huvitav koht siin
							if (is_array($val))
							{					
