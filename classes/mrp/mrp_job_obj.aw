...
class mrp_job_obj extends _int_object
{
	...
	const STATE_PAUSED = 7; // in progress but on pause
	const STATE_INPROGRESS = 3; // work is being done
	const STATE_SHIFT_CHANGE = 12; // on pause for shift changing

	protected static $planning_states = array(
...
//		$res = $this->get_first_obj_by_reltype("RELTYPE_MRP_RESOURCE");
	...
	**/
	public function set_used_material_amount(object $product, $amount, object $unit = null)
	{
		...
		if (!in_array($this->prop("state"), $applicable_states))
		{
			throw new awex_mrp_job_state("State not DONE");
		}
...
**/
	function start ($comment = "")
	{
		...
		{
			throw new awex_mrp_job_state("State must be 'PLANNED'.");
		}
...
/** Ends the job. Job must be in progress.
	...
	**/
	function done ($quantity = null, $comment = "")
	{
		...
		if (!in_array ($this->mrp_state, $applicable_states))
		{
			if ($this->mrp_resource->is_processing($this->ref()))
			...
			throw new awex_mrp_job_state("State must be 'INPROGRESS'.");
		}
...
**/
	function abort ($comment = "")
	{
		...
		if (!in_array ($this->mrp_state, $applicable_states))
		{
			throw new awex_mrp_job_state("State must be 'INPROGRESS' or 'PAUSED'.");
		}
...
**/
	function scontinue($comment = "")
	{
		...
		{
			throw new awex_mrp_job_state("Job is not ready to continue");
		}
...
**/
	function acontinue($comment = "")
	{
		...
		if (!in_array ($this->mrp_state, $applicable_states))
		{
			throw new awex_mrp_job_state("State must be 'STATE_ABORTED'.");
		}
...
/** Pauses the job for shift end. Job must be in progress.
    @attrib api=1 params=pos
	@returns void
	@comment User will be logged out when calling this method. Requires job data to be fully loaded (load_data constructor parameter or load_data() method)
	@errors
		throws awex_mrp_job_state when current job state doesn't allow pausing.
		throws awex_mrp_job_not_loaded on load error.
		throws awex_mrp_job on other errors.
...
/** Job data isn't loaded. User must call load_data() before retrying operation. **/
class awex_mrp_job_not_loaded extends awex_mrp_job {}
