...
class _int_object_loader extends core
{
	...
	function load($oid)
	{
		...
		if (!isset($GLOBALS["objects"][$oid]) || !is_object($GLOBALS["objects"][$oid]) || 
			(ifset($GLOBALS["objects"][$oid],"obj_sys_flags","no_auto_translation") != $GLOBALS["__obj_sys_opts"]["no_auto_translation"]))
		{
