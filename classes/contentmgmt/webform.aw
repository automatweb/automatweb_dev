...
------------- entries -------------
@groupinfo entries caption="N�ta sisestusi" submit=no parent=show_entries
...
------------- get_controllers -------------
@groupinfo get_controllers caption="N�tamine" parent=controllers
...
class webform extends class_base
{
	function webform()
	{
		...
		$this->trans_names = array(
			"text" => t("Tekst"),
			"textbox" => t("V�ke tekstikast"),
			"checkbox" => t("M�keruut"),
			"classificator" => t("Valikv�i"),
			"date_select" => t("Kuup�vavalik"),
			"textarea" => t("Suur tekstikast"),
			"hidden" => t("Peidetud v�i"),
			"submit" => t("Saada nupp"),
			"reset" => t("Thista nupp"),
		);
	...
	function _on_init($arr)
	{
		...
		$set_controllers = array(
			array(
				"name" => "*elemendinimi* peab olema t�detud",
				"formula" => 'if($prop["value"] == ""){$retval = PROP_ERROR;}',
				"errmsg" => "%caption peab olema t�detud",
			),
			array(
