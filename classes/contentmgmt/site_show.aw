...
//	}
	...
	function make_final_vars()
	{
		...
		$this->vars(array(
			...
			"date_timestamp" => time(),
			"date3" => date("j").". ".locale::get_lc_month(date("n"))." ".date("Y"),
			"date4" => locale::get_lc_weekday(date("w")).", ".get_lc_date(time(),LC_DATE_FORMAT_LONG_FULLYEAR),
			"date4_uc" => ucwords(locale::get_lc_weekday(date("w"))).", ".get_lc_date(time(),LC_DATE_FORMAT_LONG_FULLYEAR),
			"date5" => date("j").". ".locale::get_lc_month(date("n"))." ".date("Y"),
			"site_title" => strip_tags($this->site_title),
