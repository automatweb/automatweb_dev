...
//			$sel_lang = $langs->fetch(aw_global_get("lang_id"));
			// seems that this site has only one language, 
			// so, we can take the first object of the all_languages list:
			
			$selected_language = $all_languages->begin();
			
			// if this language isn't the active language, then make it active:
			$selected_language_lang_id = $selected_language->prop("lang_id");
			if ($lang_id != $selected_language_lang_id)
			{
				$lang_id = $selected_language_lang_id;
				$language_inst = get_instance("languages");
				$language_inst->set_active($lang_id, true);
			}
...
//			$url = $this->cfg["baseurl"] . "/?set_lang_id=$row[id]";
			$url = $this->cfg['baseurl'] . "/?set_lang_id=$language_lang_id";
...
//			$sel_lang = $ll->fetch(aw_global_get("lang_id"));
			...
			// so, seems, that if no active_language is set in above loop, then lets just take the first one in the
			// list:
			$selected_language = $all_languages->begin();
			$lang_id = $selected_language->prop("lang_id");
			$languages_inst = get_instance("languages");
			$languages_inst->set_active($lang_id, true);
		}
