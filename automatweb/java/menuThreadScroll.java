...
{//nimi 
	...
	branch(String nimi,int astak,boolean laast,Tahvel aaken,Applet bos,Color b,Color m,Color s,Color t,String closeurl,String openurl,String urll,int arv,int ooid,boolean[] jon) 
	{ 
		back=b;				mouse=m; 
		select=s;			text=t; 
		boss=bos;			aken=aaken; 
...
{ 
	...
	hiireKuular2(branch fold,Tahvel aaken,Applet bos) 
	{ 
		boss=bos; 
		aken=aaken; 
		folder=fold; 
	...
	System.out.println("ERINEVUS "+folder.label+" pidi olema "+folder.alluvaid+" alluvat, oli "+i);
			...
			{ 		
				...
				{ 
					...
					aken.setLayout(new GridLayout(i+j,1,0,0)); 
					aken.setSize(aken.getSize().width,aken.getSize().height+20*j); 
aken.ulatus=aken.getSize().height;
					aken.doLayout();
					//boss.getComponent(1).doLayout();
boss.doLayout();				
				} 			
...
class vertikaalKuular implements AdjustmentListener{
Tahvel laud;
int ulatus;//tabeli suurus

	vertikaalKuular(Tahvel lad,int ala){
		laud=lad;
		ulatus=ala;
	}

	public void adjustmentValueChanged(AdjustmentEvent e){
		int ekraan=laud.getSize().height;
		int end=laud.start.y+ulatus;
		int value=e.getValue();

		if((laud.start.y<0)&&(end<ekraan)){
			laud.start.y=-Math.round((float)value*ulatus/100);
		}
		else
		if((laud.start.y<0)&&(end>=ekraan)){
			laud.start.y=-Math.round((float)value*ulatus/100);
...
class Tahvel extends Panel 
{
	Point start;
	Scrollbar vertikaal=new Scrollbar(Scrollbar.VERTICAL);
	Applet boss;
	int i,ulatus;

	Tahvel(Applet bos,int ala)
	{
		vertikaal.setSize(20,vertikaal.getSize().height);
		ulatus=ala;
		boss=bos;
		start=new Point(0,0);
		vertikaal.addAdjustmentListener(new vertikaalKuular(this,ulatus));
	}

	public void paint(Graphics g) {
		g.translate(start.x,start.y);//paneb nähtava koha alguse paika
	 ...
	 public void fixScroll(){
		...
		Point punkt=kohenda(start.y,(start.y+ulatus),this.getSize().height);//(keritava ala algus kordinaat, keritava ala lõpu kordinaat)
		vertikaal.setVisibleAmount(punkt.y);							
		vertikaal.setValue(punkt.x);
		if(punkt.y==100){
			if(vertikaal.isValid()==true){//muidu jääb refreshimisega tsüklisse
				boss.remove(vertikaal);
				boss.doLayout();
			}
		}
		else{
			if(vertikaal.isValid()==false){
				boss.add(vertikaal,"East");//vajadusel näitan
				boss.doLayout();
			}
		}
	 }
	 ...
	 private Point kohenda(int begin,int end,int ekraan){
		int beginAbs=Math.abs(begin);
		int value=0;//mitu pixlit pilti liigutada 1pixli võrra nuppu liigutades
		int visible=100; //nupu suurus

		if((begin<0)&&(end<ekraan)){
			value=Math.round((float)beginAbs/(float)ulatus*100);
			visible=100-value;
		}
		else
		if((begin<0)&&(end>=ekraan)){
			value=Math.round((float)beginAbs/(float)ulatus*100);
			visible=Math.round((float)ekraan/(float)ulatus*100);
		}
		else
		if((begin>=0)&&(end<ekraan)){
			value=0;
		...
		if((begin>=0)&&(end>=ekraan)){
			value=0;
			visible=Math.round((float)(ekraan-begin)/ulatus*100);
		}

		return new Point(value,visible);
	 }
}


 
public class menuThreadScroll extends Applet 
{ 
	Panel aken;
	//ScrollPane scroll;
	Panel scroll;
	...
	public void init() 
	{
		...
		Tahvel aken=new Tahvel(this,600);
...
aken.setSize(getSize().width,hait*20);
aken.ulatus=hait*20;
