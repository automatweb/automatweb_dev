...
{//nimi  
	...
	{  
		...
		closed=kinni;  
		name=new Label(nimi);
		//name.setSize(name.getSize().width,name.getSize().height-4);
		//name.setSize(name.getSize().width,22);
...
/*
		...
		cc.weightx = 1.0;
		//cc.ipady=-2;
		cc.gridwidth = GridBagConstraints.RELATIVE; //end row REMAINDER
		cc.gridheight = 1;

		cc.anchor=GridBagConstraints.WEST;
		//this.setLayout(base); 
*/		
		//base.setConstraints(image, cc);

		this.add(image); 
		
		//base.setConstraints(name, cc);
...
{//nimi  
  ...
  /*
		...
		*/	
		...
		//right.name.setBackground(Color.red);
		//right.image.setBackground(Color.green);
...
{  
	...
	{	
		...
		nupp.setLayout(new FlowLayout(FlowLayout.LEFT,2,1)); 
...
//System.out.println("ikooniurl="+iconurl);
				...
				//first.setBackground(new Color(i*30,127-i*30,127+i*30));
				first.count=i;  
				//paneel.add(first);
			...
			Panel fixer=new Panel();//??? kas scrolli buhul on vaja???
			cc.weighty = 1.0;
			base.setConstraints(fixer, cc);
			aken.add(fixer);
		...
		//	} 
			...
			System.out.println("Elemente="+aken.getComponentCount());
